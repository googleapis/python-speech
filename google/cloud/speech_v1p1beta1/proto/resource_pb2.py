...
CustomClass = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      custom_class_id:
          If this custom class is a resource, the custom\_class\_id is
...
PhraseSet = _reflection.GeneratedProtocolMessageType(
    ...
    in january", "i was born in febuary", ...), use the pre-built
    ``$MONTH`` class improves the likelihood of correctly transcribing
    audio that includes months (e.g. "i was born in :math:`month"). To
    refer to pre-built classes, use the class' symbol prepended with ``\
    ``e.g.``\ $MONTH\ ``. To refer to custom classes that were defined
    inline in the request, set the class's``\ custom\_class\_id\ ``to a
    string unique to all class resources and inline classes. Then use the
    class' id wrapped in $``\ {...}``e.g. "${my-months}". To refer to
    custom classes resources, use the class' id wrapped in``\ :math:`{}`
    (e.g. ``\ {my-months}\`).
    Attributes:
        value:
            The phrase itself.
